...
kernel void FN(NOCONCURW const int *restrict param_stack,
  NOCONCURW const T *restrict amat, NOCONCURW const T *restrict bmat,
  global T *restrict cmat)
{
  const int gid = get_group_id(0);
  NOCONCURW const int *const restrict param_base = param_stack + gid * 3;
  /* indexes given by param_stack are one-based */
  const int ai = param_base[0] - 1, bi = param_base[1] - 1, ci = param_base[2] - 1;
  NOCONCURW const T *const restrict awg = amat + ai, *const restrict bwg = bmat + bi;
